...
console.log(`CantidadPalabrasEnLista: ${cantidad}`);
let aleatorio = Math.floor(Math.random() * cantidad);
let palabraActualJuego = palabrasJuego[aleatorio];
console.log(`LaPosicionDeLaAleatoriaPalabraEs: ${aleatorio}`);
console.log(`PalabraAleatoria en juego: ${palabraActualJuego}`);
...
const inputMobileListener = inputInvisible.addEventListener(
  ...
  function keyDetectorMobile(e) {
    ...
    console.log(
      "%c%s%c%s",
      "color:blue;background:yellow;font-size:30px",
      "Tenes:",
      "color:#fff;background:red;font-size:30px",
      " " + palabrasErradas.value.length + " palabras erradas"
